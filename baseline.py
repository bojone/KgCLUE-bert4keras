...
test_data = load_data('../datasets/test_public.json')
...
class AutoQA(AutoRegressiveDecoder):
    ...
    def predict(self, inputs, output_ids, states):
        ...
        probas = self.last_token(model).predict([all_token_ids, segment_ids])
    ...
    def generate(self, text, topk=1):
        ...
        return (
            tokenizer.decode(subject_ids), tokenizer.decode(predicate_ids),
            tokenizer.decode(meaning_ids), KG[output_ids[:-1]]
        )


autoqa = AutoQA(start_id=None, end_id=tokenizer._token_end_id, maxlen=maxlen)
